...
def initialize_parameters():
    ...
    W4 = tf.get_variable('W4', [80, 80], initializer=tf.contrib.layers.xavier_initializer(seed=1))
    b4 = tf.get_variable('b4', [80, 1], initializer=tf.zeros_initializer())
    WL = tf.get_variable('WL', [9, 80], initializer=tf.contrib.layers.xavier_initializer(seed=1))
    bL = tf.get_variable('bL', [9, 1], initializer=tf.zeros_initializer())
...
def forward_propagation(X, parameters, keep_prob1, keep_prob2):
    """
    Implements the forward propagation for the model: (LINEAR -> RELU)^4 -> LINEAR -> SOFTMAX
    ...
    Returns:
    ZL -- the output of the last LINEAR unit (logits)
    """
    ...
    A3 = tf.nn.relu(Z3)
    Z4 = tf.matmul(W4, A3) + b4  # Z3 = np.dot(W3,Z2) + b3
    A4 = tf.nn.relu(Z4)
    ZL = tf.matmul(WL, A4) + bL
...
def predict(X, parameters):
    ...
    WL = tf.convert_to_tensor(parameters["WL"])
    bL = tf.convert_to_tensor(parameters["bL"])
    ...
    ZL = forward_propagation(x, params, keep_prob1, keep_prob2)
    p = tf.nn.softmax(ZL, dim=0) # dim=0 because the classes are on that axis
    # p = tf.argmax(ZL) # this gives only the predicted class as output
...
def model(X_train, Y_train, X_test, Y_test, learning_rate=0.0001,
    ...
    """
    Implements a four-layer tensorflow neural network: (LINEAR->RELU)^4->LINEAR->SOFTMAX.
    ...
    # Forward propagation
    ZL = forward_propagation(X, parameters, keep_prob1, keep_prob2)
    ...
    with tf.Session() as sess:
        ...
        dirname = 'output/'
        filename = timestr + '_NN5L_lr_{}_beta_{}_testcost_{:.2f}_testacc_{:.2f}.png'.format(learning_rate, beta,
